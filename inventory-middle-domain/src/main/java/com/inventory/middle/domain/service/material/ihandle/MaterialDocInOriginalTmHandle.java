...
@Service
@Slf4j
public class MaterialDocInOriginalTmHandle implements IHandler {
    @Resource
    ...
    @StopWatchWrapper(logHead = "createMaterialDoc", msg = "原入库数据提交")
    public <T, E> HandleMessage<T, E> operation(HandleMessage<T, E> message) {
        ...
        sw1.stop();
        log.info("createMaterialDoc MaterialDocInOriginalTmHandle adjustInventoryInc数据库 {}", sw1.toString());
        Stopwatch sw2 = Stopwatch.createUnstarted();
        sw2.start();
        materialDocumentIn = materialDocCoreService.createMaterialDoc(materialDocumentIn);
        sw2.stop();
        log.info("createMaterialDoc MaterialDocInOriginalTmHandle createMaterialDoc主项 {}", sw2.toString());
        ...
        sw3.stop();
        log.info("createMaterialDoc MaterialDocInOriginalTmHandle supplementMaterialDoc子项 {}", sw3.toString());
