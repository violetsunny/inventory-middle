...
@Service
@Slf4j
public class MaterialDocInOutTmHandle implements IHandler {
    @Resource
    ...
    @StopWatchWrapper(logHead = "createMaterialDoc", msg = "出入库数据提交")
    public <T, E> HandleMessage<T, E> operation(HandleMessage<T, E> message) {
        ...
        sw_1.stop();
        log.info("createMaterialDoc MaterialDocInOutTmHandle 创建物料和库存关系 {}", sw_1.toString());

        Stopwatch sw0 = Stopwatch.createUnstarted();
        sw0.start();
        //TODO 出的在途
        sw0.stop();
        log.info("createMaterialDoc MaterialDocInOutTmHandle 调整在途 {}", sw0.toString());

        Stopwatch sw1 = Stopwatch.createUnstarted();
        sw1.start();
        materialDocumentOut = materialDocCoreService.createMaterialDoc(materialDocumentOut);
        //补充子项信息
        materialDocSubCoreService.supplementMaterialDoc(materialDocumentOut);
        sw1.stop();
        log.info("createMaterialDoc MaterialDocInOutTmHandle 生成出库凭证 {}", sw1.toString());
        ...
        sw4.stop();
        log.info("createMaterialDoc MaterialDocInOutTmHandle 调整入的在途 {}", sw4.toString());
        ...
        sw2.stop();
        log.info("createMaterialDoc MaterialDocInOutTmHandle adjustInventoryInc数据库 {}", sw2.toString());
        ...
        sw3.stop();
        log.info("createMaterialDoc MaterialDocInOutTmHandle 生成入库凭证 {}", sw3.toString());
        ...
        res.setFullMaterialDocNo(materialDocumentIn.getMaterialDocNo() + COMMA + materialDocumentOut.getMaterialDocNo());
