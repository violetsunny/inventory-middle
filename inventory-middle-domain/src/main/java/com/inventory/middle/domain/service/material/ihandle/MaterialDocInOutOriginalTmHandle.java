 ...
 *
 * @author kanglele
 * @version $Id: MaterialDocInOutOriginalTmHandle, v 0.1 2021/8/20 10:06 Exp $
 */
@Service
@Slf4j
public class MaterialDocInOutOriginalTmHandle implements IHandler {
    @Resource
    ...
    @StopWatchWrapper(logHead = "createMaterialDoc", msg = "原出入库数据提交")
    public <T, E> HandleMessage<T, E> operation(HandleMessage<T, E> message) {
        ...
        sw1.stop();
        log.info("createMaterialDoc MaterialDocInOutOriginalTmHandle 生成出库凭证 {}", sw1.toString());

        MaterialDocumentBO materialDocumentIn = req.getMaterialDocumentIn();
        Stopwatch sw2 = Stopwatch.createUnstarted();
        sw2.start();
        inventoryCoreService.adjustInventoryIncOriginal(req.getInventorySupplys());
        sw2.stop();
        log.info("createMaterialDoc MaterialDocInOutOriginalTmHandle adjustInventoryInc数据库 {}", sw2.toString());
        ...
        sw3.stop();
        log.info("createMaterialDoc MaterialDocInOutOriginalTmHandle 生成入库凭证 {}", sw3.toString());
