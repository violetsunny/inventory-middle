...
public class SnMaterialDocInConsumer implements RocketMQListener<String> {
    ...
    private static final String IDEMPOTENT_PREFIX = "mq:idempotent:snMaterialDocIn:";
    ...
    public void onMessage(String message) {
        ...
        try {
            ...
            if (!bucket.trySet("1", IDEMPOTENT_TTL_HOURS, TimeUnit.HOURS)) {
                log.info("SnMaterialDocInConsumer.onMessage: duplicate message, skip, dedupKey={}", dedupKey);
                return;
            }
            ...
            throw new BusinessException("SN物料凭证入库MQ消费失败", e);
        }
    }
}
