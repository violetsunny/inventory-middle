...
import com.inventory.middle.application.plan.plan.calculate.service.PlanGenerateApplicationService;
import com.inventory.middle.interfaces.web.plan.plan.PlanConfigConverter;
import com.inventory.middle.interfaces.web.plan.plan.PlanReportConverter;
...
public class PlanConfigController  {
    ...
    @Operation(summary = "批量导入物料计划参数")
        ...
        PlanMaterialParamImportReqBO reqBO = PlanConfigConverter.convertPlanMaterialParamImportReqDTO2BO(reqDTO, token);
    ...
    @Operation(summary = "更新物料计划参数")
        ...
        PlanMaterialParameterBO bo = PlanConfigConverter.convertPlanMaterialParamUpdateDTO2BO(reqDTO);
    ...
    @Operation(summary = "查询物料计划参数")
        ...
        PlanMaterialParamQueryReqBO reqBO = PlanConfigConverter.convertPlanMaterialParamQueryReqDTO2BO(reqDTO);

        PlanMaterialParameterBO resBO = planConfigService.queryByMaterialCodeAndLogicalPlantNo(reqBO);

        return SingleResponse.buildSuccess(PlanConfigConverter.convertPlanMaterialParamQueryResBO2VO(resBO));
    }
    ...
    @Operation(summary = "分页查询物料计划参数")
        ...
        PlanMaterialParamPageReqBO bo = PlanConfigConverter.convertPlanMaterialParamPageReqDTO2BO(reqDTO);
        ...
        List<PlanMaterialParamPageResVO> data = result.getData().stream()
                .map(d -> PlanConfigConverter.convertPlanMaterialParamPageResBO2VO(d))
    ...
    @Operation(summary = "创建计划方案")
        ...
        PlanBO bo = PlanConfigConverter.convertPlanCreateReqDTO2BO(reqDTO);
    ...
    @Operation(summary = "查询计划方案")
        ...
        UserContext userInfo = UserContextHolder.get();
        return SingleResponse.buildSuccess(PlanConfigConverter.convertPlanQueryResBO2VO(planConfigService.queryPlanById(planId, userInfo.getTenantId())));
    }
    ...
    @Operation(summary = "更新计划方案")
        ...
        PlanBO bo = PlanConfigConverter.convertPlanUpdateReqDTO2BO(reqDTO);
    ...
    @Operation(summary = "分页查询计划方案")
        ...
        PlanPageReqBO bo = PlanConfigConverter.convertPlanPageReqDTO2BO(reqDTO);

        PagedSingleResponse<PlanBO> result = planConfigService.pageQueryPlan(bo);

        List<PlanPageResVO> data = result.getData().stream()
                .map(d -> PlanConfigConverter.convertPlanPageResBO2VO(d))
                .collect(Collectors.toList());
    ...
    @Operation(summary = "切换计划方案有效状态")
        ...
        ChangeStatusPlanBO bo = PlanConfigConverter.convertPlanChangeStatusReqDTO2BO(reqDTO, userInfo);
    ...
    @RequestMapping(value = "/importPlanMaterial", method = {RequestMethod.POST})
        ...
        PlanMaterialImportReqBO reqBO = PlanConfigConverter.convertPlanMaterialImportReqDTO2BO(reqDTO, token);
    ...
    @Operation(summary = "获取计划参数枚举类列表")
        ...
        return SingleResponse.buildSuccess(PlanConfigConverter.convertPlanParamBO2VO(resBO));
    }
    ...
    @Operation(summary = "获取计划物料参数枚举类列表")
        ...
        return SingleResponse.buildSuccess(PlanConfigConverter.convertPlanMaterialParamBO2VO(resBO));
    }
    ...
    @Operation(summary = "根据租户id查询计划物料参数可调拨逻辑仓")
        ...
        QueryPlanTransferLogicalPlantsReqBO bo = PlanConfigConverter.convertQueryPlanTransferLogicalPlantNoBO(materialCodeToUse, logicalPlantNo, userInfo.getTenantId());
        PlanTransferLogicalPlantsResBO resBO = planConfigService.queryPlanTransferLogicalPlants(bo);
        return SingleResponse.buildSuccess(PlanConfigConverter.convertPlanTransferLogicalPlantsResBO2VO(resBO));
    }
