 ...
 * 日期工具类（计划模块）
 */
public class PlanDateUtils {
    public static final Logger LOGGER = LoggerFactory.getLogger(PlanDateUtils.class);
    ...
    public static long addDate(long time, int num) {
        return PlanDateUtils.addDate(new Date(time), num).getTime();
    }
    ...
    public static Boolean isOneDay(Date date1, Date date2) {
        int lastDay = PlanDateUtils.getDayOfYear(date1);
        int nowDay = PlanDateUtils.getDayOfYear(date2);
        if (lastDay == nowDay) {
            return true;
        }
        return false;
    ...
    public static long getTodayZeroHour(long time) {
        ...
        try {
            ...
            LOGGER.error("PlanDateUtils.getTodayZeroHour error", e);
            return 0;
        }
    }
